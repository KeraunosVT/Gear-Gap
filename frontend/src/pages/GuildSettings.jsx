...
// ── WHAT THIS PAGE IS ───────────────────────────────────────────────────────
// The guild_config row: everything that used to be a DISCORD_* environment
// variable or a value in shared/guild.json, none of which could be changed
// without a redeploy.
//
...
const ROLE_GROUPS = [
  ...
  },
  {
    key: 'member_role_ids',
    label: 'Roster roles',
    hint: 'Who appears in the party builder, attendance and member pickers. Empty means everyone.',
  },
];
...
function RolePicker({ roles, selected, onChange, critical }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      ...
      {orphans.map((id) => (
        <button key={id} type="button" onClick={() => toggle(id)} title="This role is not visible to the bot — click to remove"
...
export default function GuildSettings() {
  ...
  const save = async () => {
    ...
    if (changed('timezone')) {
      warnings.push(`Switching the guild timezone to ${form.timezone} re-reads every scheduled time in the new zone. Event times themselves are stored as wall-clock and do not move.`);
    }
    if (changed('tag')) {
  ...
  return (
    <PageShell maxWidth="max-w-3xl">
      <p className="text-sm text-ash mb-6">
        The house&apos;s identity and its Discord wiring. These used to be environment variables — changing them
        needed a redeploy.
      </p>
      ...
      {/* Said once, at the top, because it changes how every picker below should
          be read: an empty dropdown means "can't see", not "none exist", and
          saving in that state must not be taken as clearing anything. */}
      {!botOnline && (
        <div className="mb-6 px-5 py-3 rounded-lg border border-brass/40 bg-panel text-bone text-sm flex items-start gap-2">
          <AlertTriangle className="w-4 h-4 text-brass shrink-0 mt-0.5" />
          <span>
            The Discord bot is offline, so the role and channel lists below are empty. Stored values are still shown
            and will be kept if you save — but you can&apos;t pick a new one until the bot reconnects.
          </span>
        </div>
      )}
      ...
      <div className="space-y-6">
        ...
        <section className="panel rounded-lg p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="House name" hint="The ceremonial name shown across the site and on the browser tab.">
          ...
          <Field label="Motto" hint="One line, shown under the house name on the login page.">
            <input value={form.motto || ''} onChange={(e) => set('motto', e.target.value)} className={inputClass} />
          </Field>
          <Field label="Creed" hint="The longer passage on the login page.">
            <textarea value={form.creed || ''} onChange={(e) => set('creed', e.target.value)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Guild timezone" hint="Every scheduled time on the site is written in this zone.">
              <select value={form.timezone || ''} onChange={(e) => set('timezone', e.target.value)} className={inputClass}>
                {/* A zone that isn't on the curated list stays selectable, or
                    opening this page would silently rewrite it on save. */}
                {form.timezone && !TIMEZONE_OPTIONS.some((t) => t.value === form.timezone) && (
        ...
        <section className="panel rounded-lg p-6 space-y-5">
          ...
          <div className="px-4 py-2.5 rounded-lg border border-brass/30 bg-hall text-xs text-ash flex items-start gap-2">
            ...
            <span>
              The first two lists gate this page and the login itself, and the effect is delayed — capabilities live in
              each member&apos;s session and refresh within the hour. A save that removes your own access is refused
              rather than confirmed, because there would be no way back from inside the app.
            </span>
          </div>
        ...
        <section className="panel rounded-lg p-6 space-y-4">
          ...
          <Field label={<span className="flex items-center gap-1.5"><Volume2 className="w-3 h-3" /> Attendance voice channel</span>}
            hint="Snapped by /attendance when the officer running it isn't already in a voice channel, and named on the Attendance page. Naming a channel on the command always wins.">
