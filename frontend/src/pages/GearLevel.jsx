...
// The Watermark Upload is the in-game "Equipment Level" popup — four numbers
// the game has already worked out. Reading it is the only thing that sets a
// gear level.
//
// The Gear Screenshot Upload is the full equipment window, filed and not read.
// It used to be parsed item by item so the maxima could be recomputed with
// Heroic gear excluded, and that result overwrote the member's level — which
// meant the same column meant two different things depending on which upload
// came last, off a per-item parse too unreliable to trust without opening the
// image anyway. Now the image is simply kept, for an officer to look at.
//
// The `popup` / `window` keys below are the stored source values and are
// deliberately unchanged by the rename — they are in the database.

const SOURCE_LABEL = {
  popup: 'Watermark Upload — includes Heroic items',
  // Only on rows written before the Gear Screenshot Upload stopped setting levels.
  window: 'An older gear screenshot upload — Heroic items excluded',
};
...
export default function GearLevel() {
  ...
  // The Watermark Upload leads: it is the one that sets your gear level, so it
  // is what someone landing here needs. The Gear Screenshot Upload is a second,
  // optional thing you send afterwards.
  ...
  return (
    <PageShell maxWidth="max-w-3xl">
      <p className="text-sm text-ash mb-5">
        Send a <span className="text-bone">Watermark Upload</span> to record your levels — that&apos;s the one that gets
        read. You can also file a <span className="text-bone">Gear Screenshot Upload</span> so officers can see the
        gear behind the numbers; nothing is read out of it.
      </p>
      ...
      {loading ? (
        ...
        <>
          {/* ── UPLOAD, FIRST ──────────────────────────────────────────────
              ...
              is the same either way. Watermark first: it is the one that sets a
          ...
          <Tabs
            ...
            items={[
              { key: 'popup', label: 'Watermark Upload' },
              { key: 'window', label: 'Gear Screenshot Upload' },
            ]}
          />
          ...
          {tab === 'popup' && (
            <div className="space-y-3">
              <Dropzone onFile={(f) => post('/api/gear-ilvl', f, 'popup', 'Gear level updated.')}
                ...
                Click to upload your watermark
              </Dropzone>
              {/* The name is ours; the thing being asked for is the game's, so
                  the description still names it exactly as it appears in-game.
                  A member who doesn't recognise "watermark" needs this line. */}
              <p className="text-ash/60 text-xs">
                The small in-game Equipment Level popup showing Equipment Lv. / Max Weapon / Max Armor / Max Accessory.
                The four numbers on it become your gear level, Heroic items and all — the game counts them and there is
                no way to read the popup without them.
              </p>
            </div>
          )}
          ...
          {tab === 'window' && (
            <div className="space-y-3">
              <Dropzone onFile={(f) => post('/api/gear-screenshot', f, 'window', 'Gear screenshot saved.')}
                busy={uploading === 'window'} busyLabel="Saving screenshot…">
                Click to upload your gear screenshot
              </Dropzone>
              <p className="text-ash/60 text-xs">
          ...
          {entry ? (
            ...
            // Specific about WHICH upload, because a Gear Screenshot Upload no
            // longer produces a level — "upload a screenshot" to someone who
            // just uploaded one reads as their upload having failed.
            <div className="panel rounded-lg p-8 text-center text-ash">
              No gear level on file yet — send a Watermark Upload above.
            </div>
          )}
              ...
              {shot.image_url && (
                <div>
                  ...
                  <a href={shot.image_url} target="_blank" rel="noreferrer">
                    <img src={shot.image_url} alt="Your stored gear screenshot"
