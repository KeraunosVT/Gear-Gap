...
export default function GuildFeudRoster() {
  ...
  const rows = useMemo(() => {
    ...
    const filtered = decorated.filter(
      (p) => p.appearances >= minMet && (p.player_name || '').toLowerCase().includes(f),
    );
  ...
  return (
    <PageShell maxWidth="max-w-5xl">
      ...
      <p className="text-ash text-sm mb-6">
        <span className="font-mono text-bone">{data?.players?.length || 0}</span> players fielded
        {record && (
          <>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        <span className="text-sm text-ash">{rows.length} shown</span>
      ...
      {rows.length === 0 ? (
        <EmptyState>{filter || minMet > 1 ? 'Nobody matching.' : 'No players on record for this guild.'}</EmptyState>
      ) : (
      ...
      <p className="text-ash/60 text-xs mt-4 flex items-start gap-1.5">
        ...
        <span>
          ...
          {data?.min_appearances || 3} matches or more. The median is taken across their{' '}
          <span className="text-ash">{data?.eligible_count || 0}</span> regulars, borrowed players excluded — so an even
          guild marks nobody, which is itself worth knowing. Click any name to see where else they&apos;ve played.
        </span>
      </p>
