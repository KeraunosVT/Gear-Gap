...
module.exports = function createEventDetail(supabase, deps) {
  ...
  return {
    ...
    async detail(eventId, { viewerId = null, officer = false } = {}) {
      ...
      const [roster, ids, memberRoles, schedule] = await Promise.all([
        listMembers ? listMembers().catch((err) => { console.error('eventDetail: member list failed:', err.message); return null; }) : null,
        identities ? identities.load().catch(() => null) : null,
        rolesFor(),
        event.event_schedule_id
          ? supabase.from('event_schedule').select('name, event_time').eq('id', event.event_schedule_id).maybeSingle()
      ...
      const [unavailable, signupView, lateRequests] = await Promise.all([
        ...
        event.event_date && signups
          ? signups.forOccasion({ date: event.event_date, eventScheduleId: event.event_schedule_id || null })
            .catch((err) => { console.error('eventDetail: signup lookup failed:', err.message); return null; })
          : Promise.resolve(null),
        lateAttendance
          ? lateAttendance.list({ status: 'all', eventId })
      ...
      for (const [discordId, displayName] of everyone) {
        ...
        const base = {
          ...
          signed_up_at: entry ? entry.signed_up_at : null,
        };
        ...
        if (att) {
          ...
          lastUpdated = (req && req.decided_at) || entry.signed_up_at;
      ...
      if (viewerId) {
        ...
        const closesAt = new Date(new Date(event.created_at).getTime()
          + (lateAttendance ? lateAttendance.WINDOW_HOURS : 24) * 3_600_000);
