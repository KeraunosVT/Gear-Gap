...
- **Gear Level** — upload the small **Equipment Level popup** and its four numbers are read automatically (Gemini). That is the only thing that sets a gear level. Separately you can file a shot of your **full equipment window**: it is stored and visible to you and to officers, and nothing is read out of it — no parse, no numbers, no effect on your level. It used to be parsed item by item to recompute the maxima with Heroic gear excluded, but two uploads writing one column meant the number meant different things for different members, off a per-item read too unreliable to trust without opening the image anyway. The picture is now the evidence, and the popup is the measurement
...
- **Gear Levels / Merge Names** — guild-wide gear-level leaderboard, opening any stored equipment window to check a number against the picture. Two counts sit at the top — how many have submitted a gear level, and how many have an equipment screenshot on file — kept separate because one no longer implies the other; both narrow with the search box. Members who have filed a screenshot but never sent the popup appear with no levels and a *screenshot only* tag, rather than dropping off the table with their image unreachable; older rows measured back when the window set levels stay marked, since they were scored by a different rule. Reconcile OCR-misread in-game names to the right player
