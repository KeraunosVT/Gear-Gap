...
- **Feuds** (`/war-record/feuds`) — the head-to-head record against every guild you've met: matches, W/L/D, win %, kills for and against, last met. **Each match is credited to the one guild that fielded most of the other side**, and anyone else on it counts as having subbed for them — otherwise a guild that lent three players gets its own row for a match it never chose to fight, sitting next to guilds that brought fifty. Expand a row for a preview, or open the guild for the full sheet at `/war-record/feuds/:guild` — **Class · Player · Recent · Met · K/M · Dmg/M · Heal/M**, sortable, borrowed players tagged with their own guild. It opens on the **current roster** — whoever they fielded in the last 3 games — with an all-time toggle beside it.

  A player is marked <span title="standout">▲</span> when a per-match rate is at least **twice their own guild's median** for it, over **five matches or more**.

  **Two windows, deliberately different.** The list narrows to the last 3 games because that answers "who are they running now"; every rate and ▲ behind it comes from that player's *full* history against you, because three games is far too few to call anyone dangerous. A single window can't do both jobs — which is why the roster window is smaller than the marking floor, and a test asserts it stays that way. Not a leaderboard: crowning the top of each column marks somebody in every guild including one where five players are indistinguishable, whereas this marks three in a guild with three carries and **nobody** in an even one — which is itself the answer. The median is taken over that guild's regulars only, borrowed players excluded, so a roster padded with one-match subs can't drag it to zero and turn every regular into an outlier. Ranking is on the stat rather than the class because only 11 of the game's 45 classes have a role mapping anywhere in this app — healing output identifies a healer whatever they're playing.

  Every scoreboard already stored **both** teams; nothing read the enemy half until now. Aggregated in SQL (`migrations/019`–`020`) because answering it in JS would mean paging every player row of every match on each load. Officers holding `names` also get merge controls: a misread rival (`lron Vow` for `Iron Vow`) is *suggested* by name similarity and merged only on confirmation, into a **separate** `enemy_guild_aliases` table — see below.
...
- **Leave of Absence** — submit LOA for a single event, a date range, or recurring days (pick more than one at once); optionally scope any of these to a time window (e.g. "I can make the 6pm event but I'm out after that," or "out 7–8pm, back after") — also via `/loa` in Discord. Cancelling one deletes its announcement, so a nominated member can be [DMed](#guild-settings-adminsettings) when it happens: coming back otherwise *removes* evidence rather than adding it, and nobody planning a night would notice.
  ...
  **One night off a recurring LOA** — a standing "out every Tuesday" is projected onto every Tuesday on the board, so each night reads as its own row. The X on one of those now removes **just that night** (`DELETE /api/loa/:id/occurrences/:date`, stored as `skip_dates` on the entry); it used to cancel the entire series, silently and with nothing to undo it. Ending the whole rule is still there as a separate, confirmed button that names what it will do. A removed night is listed under the member's Recurring row and in My LOAs, where one click puts it back — and it gets the same DM as a cancellation, because a change that only ever *removes* a name from the board is invisible to whoever is planning that night. Past exceptions are never pruned: `unavailableOn()` is asked about finished nights too, and dropping them would rewrite who was absent
...
- **Where You Stand** — a panel at the top of the dashboard answering "what do I owe": events you're signed up for, LOAs still on file, your 30-day attendance rate, and your gear level with its age. It surfaces the two things that otherwise expire quietly — a night the attendance snapshot missed you on **while the 24-hour claim window is still open**, and a gear level over 30 days old. Assembled in the browser from four endpoints that are already scoped to the caller, so it adds no new read and no new place for one member's data to reach another's screen
...
- **Who's Owed** — attendance against what each member has actually received, sortable on every column: rate, nights, items (broken out by the build each was tagged for), Lucent and shards. The three inputs were always stored and nothing joined them, so *"who has shown up the most and received the least"* was settled from memory across three pages. **The window governs both halves** — a 30-day rate beside an all-time loot count isn't a comparison, it's a way to make a long-standing member look greedy. Gated on `loot.awards`; an officer without `loot.currency` gets the table with the currency columns omitted rather than zeroed
...
**Enemy players** (`enemy_player_aliases`, migration 021) work identically one level down, and matter more than they look: OCR turning one person into `Ravager` / `Ravag3r` / `Ravaqer` gives three rows with a third of the matches each — which also drops all three under the standout floor, so **the misread hides exactly the player it fragments**. Merged from a guild's roster page, suggested within that one guild only (across the whole record, unrelated players with similar names would be proposed constantly). A name belonging to a **current** member is refused in both directions — current members are merged on the Names page, and a name handled by two systems diverges the moment either is edited. A **former** member is not blocked: identity rows are never deleted (deliberately — they keep old match rows readable), so having one only means "we knew this name once", and someone who left and now plays for a rival genuinely is an enemy player. "Current" means membership of the Discord guild, read at merge time, so rejoining re-protects the name. If Discord can't be reached the merge is refused rather than guessed at — the same call `assertAliasesSafe` makes when it can't check the war record. Matching is case-insensitive, with a unique index on `lower(alias)` so `Vex` and `vex` can't point different ways.
